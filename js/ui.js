(function() {
    ...
    function renderOutput(PIs, EPIs, solutionMap, resultExpr, stepsContainer) {
        ...
        let eqHtml = solutionMap.map((item, i) => {
            return `<span class="inline-block cursor-pointer px-1 transition-all rounded" 
                          style="color: ${item.color}"
                          onmouseenter="highlightGroup(${i})" onmouseleave="unhighlightGroup()">
        ...
        solutionMap.forEach((item, i) => {
            ...
            stepsHtml += `
                <div class="utility-card p-3 flex items-center justify-between cursor-pointer group"
                     onmouseenter="highlightGroup(${i})" onmouseleave="unhighlightGroup()">
    ...
    window.highlightGroup = function(idx) {
    ...
    window.unhighlightGroup = function() {
        document.querySelectorAll('.svg-loop').forEach(el => {
